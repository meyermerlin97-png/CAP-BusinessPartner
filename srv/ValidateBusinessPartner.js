...
const DEBUG = cds.debug('validate')

module.exports = class ValidateBusinessPartner extends cds.ApplicationService {
    async init() {
        const { BusinessPartners } = this.entities
        const messaging = await cds.connect.to('messaging')

        this.on('SAVE', BusinessPartners, this.sendBP)
        messaging.on('businessPartners/created', this.handleValidation.bind(this))
    ...
    async handleValidation(req) {
        const { BusinessPartners } = cds.entities('de.cronos.businesspartner')
        const { ID } = req.data
        if (!ID) return
        const bp = await SELECT.from(BusinessPartners, { ID }, (bp) => {
            ;(bp('*'),
                bp.AddressData((a) => {
                    a('*')
                }),
                bp.CompanyCodeData((c) => {
                    c('*')
                }),
                bp.SalesAreaData((s) => {
                    s('*')
        ...
        if(bp.validationStatus_ID === 10) { return }

        DEBUG?.('Handling validation for Business Partner:', bp)

        return UPDATE(BusinessPartners, { ID }).set({ validationStatus_ID: 10 })
    }
}
